...
describe("DID JWT Tests", () => {
  ...
  it("JWT verification pass", async () => {
    ...
    expect(verificationResult).eql({
      ...
      iat: 1605571410,
      issuer:
        "did:jwk:ZUp3bGprMFRRa0FBUVAvTG5qT2pRdWxXYklPeWtSajIwb2dkSDV2S1JxVHB2N2" +
  ...
  it("JWT sign by cert should pass", async () => {
    const jwkResolver = getResolver();
    const resolver = new Resolver({
      jwk: jwkResolver,
    });
    ...
    const verificationResult = await DIDJwt.verify(resolver, JWT_3, caStore);
